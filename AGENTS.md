...
- Window ledger: `../wakeflow-ledger/AlembicTest`
...
4. If there is a current plan, task package, or direct-thread delivery, execute only the content under `../.workspace-active/workspace/current` explicitly assigned to `AlembicTest`.
...
- This window only handles dispatch packets for `AlembicTest` and returns `TargetResultEnvelope`. Do not claim, accept, or process other window tasks.
...
- Non-Test windows must not create, process, or verify AlembicTest delivery unless both the current plan and delivery envelope explicitly authorize it.
...
- Long-term cross-repository collaboration docs, plans, acceptance records, scans, and boundary records go to `../wakeflow-ledger/AlembicTest`. This repository `docs/` is only for product, release, or user docs maintained with the source.
<!-- wakeflow:scope:end -->

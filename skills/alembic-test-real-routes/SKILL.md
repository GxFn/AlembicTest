...
```bash
node AlembicTest/scripts/verify-test-environment.mjs --project BiliDili --json
node AlembicTest/scripts/verify-test-environment.mjs --url <dashboardUrl> --json
```
...
node AlembicTest/scripts/probe-cold-start-process-timeline.mjs \
  ...
  --timeout-ms 180000 \
  --poll-ms 2500 \
  --output AlembicTest/tmp/<task-id>-cold-start.json
```
...
## Fresh Runtime / Agent Dist Route

When the test involves Alembic runtime consuming `@alembic/agent`:
...
If source evidence exists but fresh dist proof or runtime linkage is missing, report stale dist / runtime linkage risk instead of product failure.
...
- source commit, fresh dist proof, and runtime linkage when relevant;
